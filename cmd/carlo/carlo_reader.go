...
func main() {
	...
	readUint32, err = client.ReadUint32(unitId, 0x000c, modbus.LITTLE_ENDIAN, modbus.LOW_WORD_FIRST, modbus.INPUT_REGISTER)
	if err != nil {
		return
	}
	fmt.Printf("Little, LWF, %d, %d\n", readUint32, int32(readUint32))

	readUint32, err = client.ReadUint32(unitId, 0x000c, modbus.BIG_ENDIAN, modbus.HIGH_WORD_FIRST, modbus.INPUT_REGISTER)
	if err != nil {
		return
	}
	fmt.Printf("Big, HWF, %d, %d\n", readUint32, int32(readUint32))

	readUint32, err = client.ReadUint32(unitId, 0x000c, modbus.LITTLE_ENDIAN, modbus.HIGH_WORD_FIRST, modbus.INPUT_REGISTER)
	if err != nil {
		return
	}

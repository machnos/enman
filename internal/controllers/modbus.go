	...
	if config.Brand == "Victron" || config.Brand == "" {
		...
		if log.InfoEnabled() {
			if modbusClient.Speed() > 0 {
				log.Infof("Probing for Victron grid controller with baud rate %d at %s", modbusClient.Speed(), modbusClient.URL())
			} else {
				log.Infof("Probing for Victron grid controller at %s", modbusClient.URL())
			}
		}
